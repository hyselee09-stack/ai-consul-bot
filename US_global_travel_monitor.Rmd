...
# ==============================================================================
# 글로벌 안전 등급 모니터링 (이전 기록과 비교)
# ==============================================================================
print(paste("🌍 [Global Watch]", Sys.time(), "여행 경보 스캔 및 비교 중..."))
...
csv_file <- "travel_status_memory.csv" # 기록을 저장할 파일명
...
tryCatch({
  ...
  if (file.exists(csv_file)) {
    ...
    comparison <- current_data %>%
      ...
      filter(Level_Num_new != Level_Num_old) # 등급 숫자가 바뀐 것만 추출
    ...
    if (nrow(comparison) > 0) {
      ...
      for(i in 1:nrow(comparison)) {
        ...
        if (row$Level_Num_new > row$Level_Num_old) {
          status <- "🚨 위험 증가 (Worsened)"
          emoji <- "📉"
        } else {
          status. <- "✅ 상황 호전 (Improved)"
          emoji <- "📈"
        }
        ...
        msg <- paste0(
          ...
          "🏳️ 국가: ", row$Country, "\n",
          emoji, " 상태: ", status, "\n",
          "🔢 등급 변화: Lv.", row$Level_Num_old, " ➡️ Lv.", row$Level_Num_new, "\n",
          "📝 내용: ", row$Level_Text_new, "\n",
          "🔗 상세: ", row$item_link_new
        )
        
        try(bot$sendMessage(chat_id = MY_CHAT_ID, text = msg))
        Sys.sleep(1)
      }
      print(paste("✅ 변경 사항", nrow(comparison), "건 알림 전송 완료"))
    ...
    print("   -> [초기화] 과거 데이터 파일이 없습니다. 새로 생성합니다.")
  }
  
  # 5. 현재 데이터를 파일로 저장 (다음 실행을 위해 업데이트)
  write_csv(current_data, csv_file)
  print(paste("💾 현재 상태 저장 완료:", csv_file))
