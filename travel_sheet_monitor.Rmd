---
title: "Global Travel Monitor (Sheet Import Method)"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
if (!require("pacman")) install.packages("pacman")
pacman::p_load(googlesheets4, tidyverse, telegram.bot, lubridate, stringr, readr)


TELEGRAM_BOT_TOKEN <- Sys.getenv("TELEGRAM_BOT_TOKEN")
MY_CHAT_ID         <- Sys.getenv("MY_CHAT_ID")
bot <- Bot(token = TELEGRAM_BOT_TOKEN)


print(paste("📊 [Sheet Monitor]", Sys.time(), "구글 시트 데이터(IMPORTFEED) 확인 중..."))

# [중요] IMPORTFEED 함수가 적힌 구글 시트 주소
sheet_url <- "https://docs.google.com/spreadsheets/d/1YyHqTxOSQc4vyZbC5BlzOG_CZyXqSt7YrnrtXi5AxxY/edit?gid=0#gid=0"
csv_file  <- "travel_status_memory.csv"

# 인증 없이 읽기 모드
gs4_deauth()

tryCatch({
  # 1. 구글 시트 읽기
  # IMPORTFEED로 가져온 데이터는 컬럼 이름이 보통 Title, Author, URL, Date... 순서임
  raw_data <- read_sheet(sheet_url)
  
  if (nrow(raw_data) == 0) {
    print("⚠️ 시트가 비어있습니다.")
  } else {
    
    # 2. 데이터 정리
    current_data <- raw_data %>%
      select(1, 3, 4) %>% # 제목(Title), URL, 날짜(Date Created) 열 선택
      rename(Title = 1, Link = 2, Date_Raw = 3) %>%
      mutate(
        Country = str_trim(str_extract(Title, "^.*?(?=\\s-\\sLevel)")),
        Level_Num = as.numeric(str_extract(Title, "(?<=Level\\s)[0-9]")),
        Level_Text = str_trim(str_extract(Title, "(?<=Level\\s).*"))
      ) %>%
      filter(!is.na(Country) & !is.na(Level_Num)) %>%
      select(Country, Level_Num, Level_Text, Link)

    print(paste("✅ 데이터 로드 성공: 총", nrow(current_data), "개국"))

    # 3. 과거 기록(CSV)과 비교 (변화 감지)
    if (file.exists(csv_file)) {
      past_data <- read_csv(csv_file, show_col_types = FALSE)
      
      comparison <- current_data %>%
        inner_join(past_data, by = "Country", suffix = c("_new", "_old")) %>%
        filter(Level_Num_new != Level_Num_old)
        
      if (nrow(comparison) > 0) {
        for(i in 1:nrow(comparison)) {
          row <- comparison[i, ]
          
          if (row$Level_Num_new > row$Level_Num_old) {
             emoji <- "📉 (악화)"
             status_msg <- "위험 증가"
          } else {
             emoji <- "📈 (호전)"
             status_msg <- "상황 개선"
          }
          
          msg <- paste0(
            "🇺🇸 [여행경보 변경]\n\n",
            "🏳️ 국가: ", row$Country, "\n",
            emoji, " 상태: ", status_msg, "\n",
            "🔢 등급: Lv.", row$Level_Num_old, " ➡️ Lv.", row$Level_Num_new, "\n",
            "📝 내용: ", row$Level_Text_new, "\n",
            "🔗 링크: ", row$Link_new
          )
          
          try(bot$sendMessage(chat_id = MY_CHAT_ID, text = msg))
          Sys.sleep(1)
        }
        print(paste("✅ 알림 전송 완료:", nrow(comparison), "건"))
      } else {
        print("   -> 변동 사항 없음.")
      }
    } else {
      print("   -> [초기화] 첫 실행. 기준 데이터 저장.")
      try(bot$sendMessage(chat_id = MY_CHAT_ID, text = "✅ [여행경보] 구글 시트 연동 초기화 완료."))
    }
    
    # 4. 현재 상태 저장 (다음 비교를 위해)
    write_csv(current_data, csv_file)
    print("💾 데이터 저장 완료")
  }

}, error = function(e) {
  print(paste("❌ 에러 발생:", e$message))
})
