...
# 최종 정제: 메뉴 덩어리 제거 로직 (가장 긴 정규표현식)
    final_g4k_clean <- g4k_content %>%
        # ⭐⭐ 1. JS 코드와 헤더 메뉴를 공격적으로 제거 (추가) ⭐⭐
        str_remove("^.*?이 누리집은 대한민국 공식 전자정부 누리집입니다\\.") %>%
        # 2. 본문 이전의 모든 메뉴 링크 제거 (기존 로직 유지)
        str_remove("^.*?여권 정보 증명서 발급 발급 예외적 여권사용 허가 신청 신청 공증·아포스티유") %>%
        # 3. 문서 끝의 약관/문의 푸터 제거 (이 부분이 아직 남아있을 수 있음)
        str_remove("이용약관 개인정보처리방침 시스템 문의 : \\+82.*?\\(1:1 문의\\)") %>% 
        str_remove_all("translation日本語.*?프로그램 다운로드") %>% # 다국어 번역 위젯 잔여물 제거
